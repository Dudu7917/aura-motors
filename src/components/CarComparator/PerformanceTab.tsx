...
export default function PerformanceTab({ car1, car2 }: PerformanceTabProps) {
  const [playingAudio, setPlayingAudio] = useState<'car1' | 'car2' | null>(null);

  const handlePlayRoar = async (car: Car, slot: 'car1' | 'car2') => {
    if (playingAudio) return;
    setPlayingAudio(slot);
    try {
      await playV12Roar(car.specs.power || 150);
    } catch (e) {
  ...
  const specsList = [
    ...
    },
    {
      ...
      calc1: (1500 / car1.specs.weight) * 60,
      calc2: (1500 / car2.specs.weight) * 60,
    },
  ];

  return (
    <div className="space-y-6">
      {/* Audio & Quick telemetry comparison header */}
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {/* Car 1 Sound & Badge */}
        <motion.div
          whileHover={{ y: -2 }}
          className="rounded-2xl border border-white/10 bg-gradient-to-br from-zinc-900/90 to-zinc-950/90 p-4 relative overflow-hidden"
        >
          <div className="flex items-center justify-between">
            <div>
              ...
              <h4 className="font-display text-xs sm:text-sm font-bold text-white truncate max-w-[200px]">
                {car1.name}
              </h4>
            </div>
            <button
              type="button"
              onClick={() => handlePlayRoar(car1, 'car1')}
              ...
              className={`flex items-center gap-1.5 px-3 py-1.5 rounded-xl font-mono text-[9px] font-bold uppercase tracking-wider transition-all cursor-pointer ${
                playingAudio === 'car1'
                  ...
                  : 'bg-zinc-800 hover:bg-amber-500/20 text-amber-400 border border-amber-500/30'
              }`}
            >
              <Volume2 className={`h-3.5 w-3.5 ${playingAudio === 'car1' ? 'animate-bounce' : ''}`} />
              <span>{playingAudio === 'car1' ? 'ACELERANDO...' : 'RONCO DO MOTOR'}</span>
            </button>
          </div>
        ...
        >
          <div className="flex items-center justify-between">
            ...
            <button
              type="button"
              onClick={() => handlePlayRoar(car2, 'car2')}
              disabled={playingAudio !== null}
              className={`flex items-center gap-1.5 px-3 py-1.5 rounded-xl font-mono text-[9px] font-bold uppercase tracking-wider transition-all cursor-pointer ${
                playingAudio === 'car2'
                  ? 'bg-amber-500 text-black shadow-lg shadow-amber-500/30 animate-pulse'
            ...
            >
              <Volume2 className={`h-3.5 w-3.5 ${playingAudio === 'car2' ? 'animate-bounce' : ''}`} />
              <span>{playingAudio === 'car2' ? 'ACELERANDO...' : 'RONCO DO MOTOR'}</span>
            </button>
          </div>
          <div className="mt-3 flex items-center justify-between text-[10px] font-mono text-zinc-400 border-t border-white/5 pt-2">
            <span>Quilometragem:</span>
      ...
      <div className="space-y-4 rounded-3xl bg-zinc-900/50 border border-white/10 p-5 backdrop-blur-md">
            ...
            return (
              ...
              >
                ...
                <div className="flex items-center justify-between mb-2">
                  ...
                  <div className="flex items-center space-x-6 font-mono text-xs font-bold">
                    {/* Car 1 value */}
