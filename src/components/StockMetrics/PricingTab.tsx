  ...
  return (
    ...
    >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div className="lg:col-span-7 rounded-3xl border border-white/10 bg-gradient-to-br from-zinc-900/80 via-zinc-900/40 to-zinc-950/90 p-6 sm:p-7 backdrop-blur-2xl luxury-card-shadow relative overflow-hidden">
            ...
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={stats.priceTiers} margin={{ top: 10, right: 10, left: -10, bottom: 20 }}>
                ...
                <XAxis 
                  ...
                  tick={{ fontSize: 10, fill: 'var(--chart-text-color)' }} 
                  angle={-15} 
                  textAnchor="end"
                />
                <YAxis stroke="var(--chart-text-color)" tick={{ fontSize: 11, fill: 'var(--chart-text-color)' }} />
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-2.5 mt-4 pt-4 border-t border-white/5 relative z-10">
            {stats.priceTiers.map((tier) => (
              <div key={tier.name} className="p-3 rounded-2xl bg-zinc-950/60 border border-white/5 font-mono text-[10px] hover:border-white/15 transition-colors">
                ...
                <div className="flex justify-between text-zinc-400">
                  ...
                  <strong className="text-amber-500 font-bold">{tier.percentage}%</strong>
                </div>
                <div className="text-[9px] font-mono text-zinc-400 mt-1 whitespace-nowrap">
                  Total: <strong className="text-zinc-200">{formatBRL(tier.value)}</strong>
                </div>
              </div>
            ))}
          </div>
        ...
        <div className="lg:col-span-5 rounded-3xl border border-white/10 bg-gradient-to-br from-zinc-900/80 via-zinc-900/40 to-zinc-950/90 p-6 sm:p-7 backdrop-blur-2xl luxury-card-shadow flex flex-col justify-between relative overflow-hidden">
          ...
          <div>
            ...
            <div className="h-60 w-full relative z-10">
              ...
              <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none">
                <span className="font-luxury text-2xl text-zinc-100 font-bold">{stats.totalCars}</span>
            ...
            {stats.bodyTypes.map((bt, idx) => (
              <div key={bt.name} className="flex items-center justify-between text-xs font-mono p-1.5 rounded-xl hover:bg-white/5 transition-colors">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <strong className="text-amber-500">{bt.percentage}%</strong>
                </div>
              </div>
            ))}
          </div>
      ...
      <div className="rounded-3xl border border-white/10 bg-gradient-to-br from-zinc-900/80 via-zinc-900/40 to-zinc-950/90 p-6 sm:p-7 backdrop-blur-2xl luxury-card-shadow relative overflow-hidden">
        ...
        <div className="h-64 w-full relative z-10">
          <ResponsiveContainer width="100%" height="100%">
            <AreaChart data={stats.yearDistribution} margin={{ top: 10, right: 20, left: -10, bottom: 0 }}>
              ...
              <XAxis dataKey="year" stroke="var(--chart-text-color)" tick={{ fontSize: 11, fill: 'var(--chart-text-color)' }} />
              <YAxis stroke="var(--chart-text-color)" tick={{ fontSize: 11, fill: 'var(--chart-text-color)' }} />
              <Tooltip content={<CustomChartTooltip />} />
              <Area 
                type="monotone" 
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        {/* Combustível */}
        <div className="p-6 rounded-3xl border border-white/10 bg-gradient-to-br from-zinc-900/80 to-zinc-950/90 backdrop-blur-2xl luxury-card-shadow">
          ...
          <div className="space-y-2.5">
            {stats.fuels.map(f => (
              <div key={f.name} className="p-3.5 rounded-2xl bg-zinc-950/60 border border-white/5 flex items-center justify-between text-xs font-mono hover:border-emerald-500/30 transition-colors">
                <span className="text-zinc-300 font-semibold">{f.name}</span>
                <div className="flex items-center gap-3">
                  <span className="text-zinc-500">{f.value} carros</span>
                  <span className="text-emerald-500 font-bold">{f.percentage}%</span>
                </div>
              </div>
            ))}
          </div>
        ...
        <div className="p-6 rounded-3xl border border-white/10 bg-gradient-to-br from-zinc-900/80 to-zinc-950/90 backdrop-blur-2xl luxury-card-shadow">
          ...
          <div className="space-y-2.5">
            {stats.transmissions.map(t => (
              <div key={t.name} className="p-3.5 rounded-2xl bg-zinc-950/60 border border-white/5 flex items-center justify-between text-xs font-mono hover:border-blue-500/30 transition-colors">
                <span className="text-zinc-300 font-semibold">{t.name}</span>
                <div className="flex items-center gap-3">
                  <span className="text-zinc-500">{t.value} carros</span>
                  <span className="text-blue-500 font-bold">{t.percentage}%</span>
                </div>
              </div>
            ))}
          </div>
