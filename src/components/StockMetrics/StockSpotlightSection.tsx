...
import { Crown, Tag, Gauge, Sparkles, Flame, Zap, ArrowUpRight } from 'lucide-react';
...
export default function StockSpotlightSection({
  ...
  const spotlights = [
    ...
    },
    {
      id: 'lowest-km',
      title: 'Menor KM',
      subtitle: 'Mais Pouco Rodado',
      car: stats.lowestKmCar || stats.topValuedCar,
        ...
        {spotlights.map((item, idx) => {
          ...
          return (
            <motion.div
              ...
              className={`group rounded-3xl border border-white/10 bg-gradient-to-b from-zinc-900/80 via-zinc-900/40 to-zinc-950/90 p-4.5 backdrop-blur-2xl ${item.borderColor} transition-all duration-300 cursor-pointer flex flex-col justify-between space-y-3 luxury-card-shadow relative overflow-hidden`}
            >
              ...
              <div>
                ...
                {/* Nome do Carro */}
                <h4 className="font-display text-xs sm:text-sm font-bold text-zinc-100 line-clamp-2 group-hover:text-amber-500 transition-colors leading-snug">
                  {car.name}
                </h4>
              </div>
